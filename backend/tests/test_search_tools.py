...
from tests.fixtures import EXPECTED_RESPONSES, TEST_QUERIES, TestDataFixtures
...
class TestCourseSearchTool:
    ...
    def test_execute_with_both_filters(self, search_tool, mock_vector_store):
        ...
        mock_vector_store.search.assert_called_once_with(
            query="test query", course_name="Test Course", lesson_number=2
        )
        assert isinstance(result, str)
    ...
    def test_execute_empty_results_with_filters(self, search_tool, mock_vector_store):
        ...
        result = search_tool.execute(
            "test", course_name="Nonexistent Course", lesson_number=5
        )
