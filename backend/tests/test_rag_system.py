...
from tests.fixtures import TestDataFixtures, TEST_QUERIES
...
class TestRAGSystem:
    ...
    def test_add_course_document_success(self, rag_system):
        ...
        rag_system._mock_document_processor.process_course_document.return_value = (mock_course, mock_chunks)
        
        # Execute
        result_course, chunk_count = rag_system.add_course_document("/path/to/course.pdf")
        
        # Verify
        assert result_course == mock_course
        assert chunk_count == len(mock_chunks)
        
        rag_system._mock_document_processor.process_course_document.assert_called_once_with("/path/to/course.pdf")
        rag_system._mock_vector_store.add_course_metadata.assert_called_once_with(mock_course)
        rag_system._mock_vector_store.add_course_content.assert_called_once_with(mock_chunks)
    
    def test_add_course_document_error(self, rag_system):
        """Test course document addition with error"""
        rag_system._mock_document_processor.process_course_document.side_effect = Exception("Processing error")
    ...
    def test_add_course_folder_success(self, mock_listdir, mock_exists, rag_system):
        ...
        assert rag_system._mock_document_processor.process_course_document.call_count == 2
    ...
    @patch('rag_system.os.path.exists')
    @patch('rag_system.os.listdir')
    def test_add_course_folder_with_clear_existing(self, mock_listdir, mock_exists, rag_system):
    ...
    @patch('rag_system.os.path.exists')
    @patch('rag_system.os.listdir')
    def test_add_course_folder_skip_existing(self, mock_listdir, mock_exists, rag_system):
        ...
        rag_system._mock_document_processor.process_course_document.return_value = (mock_course, mock_chunks)
        rag_system._mock_vector_store.get_existing_course_titles.return_value = ["Existing Course"]
    ...
    def test_query_with_session(self, rag_system):
        ...
        rag_system._mock_session_manager.get_conversation_history.return_value = conversation_history
        rag_system._mock_ai_generator.generate_response.return_value = "AI response with context"
        ...
        rag_system._mock_session_manager.get_conversation_history.assert_called_once_with(session_id)
    ...
    def test_outline_tool_integration(self, rag_system):
        ...
        # Tool should be registered in tool manager
        assert rag_system.tool_manager.tools['get_course_outline'] == rag_system.outline_tool
    ...
    def test_end_to_end_query_flow(self, rag_system):
        ...
        rag_system._mock_ai_generator.generate_response.return_value = "The introduction covers basic concepts."
    ...
    def test_error_handling_in_query(self, rag_system):
        ...
        rag_system._mock_ai_generator.generate_response.side_effect = Exception("AI service error")
